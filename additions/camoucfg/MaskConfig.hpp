...
inline const nlohmann::json& GetJson() {
  ...
  std::call_once(initFlag, []() {
    ...
    // reynard: prefer the REYNARD_CONFIG_<n> chunked env, falling back to the
    // upstream CAMOU_CONFIG_<n> name so a transitional guise that still exports
    // CAMOU_CONFIG keeps working. The first non-empty family wins.
    for (const char* prefix : {"REYNARD_CONFIG_", "CAMOU_CONFIG_"}) {
      if (!jsonString.empty()) break;
      int index = 1;
      while (true) {
        std::string envName = std::string(prefix) + std::to_string(index);
    ...
    if (jsonString.empty()) {
      // Single-variable form: REYNARD_CONFIG, then the upstream CAMOU_CONFIG.
      auto cfg = get_env_utf8("REYNARD_CONFIG");
      if (!cfg) cfg = get_env_utf8("CAMOU_CONFIG");
      if (cfg) jsonString = *cfg;
    }
    ...
    if (!nlohmann::json::accept(jsonString)) {
      printf_stderr("ERROR: Invalid JSON passed to REYNARD_CONFIG/CAMOU_CONFIG!\n");
      jsonConfig = nlohmann::json{};
      return;
    }
